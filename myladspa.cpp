...
#define mydsp Faust_system_compressor 
...
class portData : public UI
{
 ...
 public:
  ...
  float *getInputZone(int num){
    return fPortZone[fInsCount + fOutsCount + num];
  }

  float *getOutputZone(int num){
    return fPortZone[fInsCount + fOutsCount + num_in_zones + num];
  }

	//--------------------------------Collect the audio ports-------------------------------

  portData(int ins, int outs) : UI(), fInsCount(ins), fOutsCount(outs), fCtrlCount(0), num_in_zones(0) {};
	virtual ~portData() {}
  ...
  virtual void addNumDisplay(const char* label, float* zone, int precision) 						{ addZone(zone);                num_in_zones--; }
	...
	virtual void addHorizontalBargraph(const char* label, float* zone, float min, float max) 			{ addZone(zone); num_in_zones--;}
...
{
        dsp*		p = create_shared_dsp(); //new mydsp();
...
#if 0
extern void start_program(int argc, char **argv, void *compressor);
#endif
...
float COMPRESSOR_get_ladspa_parameter(void *Instance,int num){
  PLUGIN* p = (PLUGIN*) Instance;
  portData *port_data = p->fPortData;

  float *controller = port_data->getInputZone(num);
  return *controller;
}

void COMPRESSOR_set_ladspa_parameter(void *Instance,int num,float value){
  PLUGIN* p = (PLUGIN*) Instance;
  portData *port_data = p->fPortData;

  float *controller = port_data->getInputZone(num);
  fprintf(stderr,"port_data: %p, controller: %p, num_zones: %d, num: %d\n",port_data,controller,port_data->fCtrlCount,num);
  *controller = value;
  //printf("Setting controller %d (%p) to %f (%f)\n",num,controller,value,*controller);
}

float COMPRESSOR_get_ladspa_graph_value(void *Instance, int num){
  PLUGIN* p = (PLUGIN*) Instance;
  portData *port_data = p->fPortData;

  float *controller = port_data->getOutputZone(num);
  return *controller;
}

void COMPRESSOR_delete_ladspa(void *Instance){
  //PLUGIN* p = (PLUGIN*) Instance;
  // fix.
}
